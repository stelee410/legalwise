...
export const getGeminiResponse = async (messages: { role: 'user' | 'assistant', content: string }[], systemInstruction?: string) => {
  if (!apiKey) {
  ...
  const model = ai.models.generateContent({
    ...
    config: {
      systemInstruction: systemInstruction || "你是一个专业的中国法律助手。请用简洁、专业且易懂的语言回答用户的法律问题。如果问题涉及复杂的法律诉讼，请建议用户咨询专业律师。",
    }
  });
